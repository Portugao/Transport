...
abstract class AbstractTableController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('mutransportmodule_table_' . $templateParameters['routeArea'] . 'view');
    }
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'mutransportmodule_table_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        ...
        foreach ($templateParameters['items'] as $table) {
            if (!$permissionHelper->hasEntityPermission($table, $permLevel)) {
                continue;
            }
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_transport_module.form.handler.table');
     ...
     *
     * @param Request $request Current request instance
     * @param integer $id Identifier of treated table instance
     *
     * @return Response Output
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if table to be deleted isn't found
     * @throws RuntimeException      Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    ...
    {
        $table = $this->get('mu_transport_module.entity_factory')->getRepository('table')->selectById($id);
        if (null === $table) {
            throw new NotFoundHttpException($this->__('No such table found.'));
        }
        ...
        $logArgs = ['app' => 'MUTransportModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'table', 'id' => $table->getKey()];
        
        // determine available workflow actions
        $workflowHelper = $this->get('mu_transport_module.workflow_helper');
        $actions = $workflowHelper->getActionsForObject($table);
        ...
        $redirectRoute = 'mutransportmodule_table_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this table.'));
            $logger->error('{app}: User {user} tried to delete the {entity} with id {id}, but this action was not allowed.', $logArgs);
        
            return $this->redirectToRoute($redirectRoute);
        }
        
        $form = $this->createForm(DeletionType::class, $table);
        if ($table->supportsHookSubscribers()) {
            $hookHelper = $this->get('mu_transport_module.hook_helper');
        
            // Call form aware display hooks
            $formHook = $hookHelper->callFormDisplayHooks($form, $table, FormAwareCategory::TYPE_DELETE);
        }
        ...
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                if ($table->supportsHookSubscribers()) {
                    ...
                    $validationErrors = $hookHelper->callValidationHooks($table, UiHooksCategory::TYPE_VALIDATE_DELETE);
                    if (count($validationErrors) > 0) {
                        ...
                        $success = $workflowHelper->executeAction($table, $deleteActionId);
                        if ($success) {
                        ...
                        if ($table->supportsHookSubscribers()) {
                            ...
                            $hookHelper->callFormProcessHooks($form, $table, FormAwareCategory::TYPE_PROCESS_DELETE);
                        
                            // Let any ui hooks know that we have deleted the table
                            $hookHelper->callProcessHooks($table, UiHooksCategory::TYPE_PROCESS_DELETE);
                        }
    ...
    {
        return $this->getTablesInternal($request, true);
    }
    ...
    {
        return $this->getTablesInternal($request, false);
    }
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if table to be displayed isn't found
     */
    ...
    protected function displayInternal(Request $request, TableEntity $table, $isAdmin = false)
    {
        ...
        $controllerHelper = $this->get('mu_transport_module.controller_helper');
        $templateParameters = $controllerHelper->processDisplayActionParameters($objectType, $templateParameters, $table->supportsHookSubscribers());
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUTransportModule', 'user' => $userName, 'action' => $action, 'entity' => 'table', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUTransportModule', 'user' => $userName, 'entity' => 'table', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUTransportModule', 'user' => $userName, 'action' => $action, 'entity' => 'table', 'id' => $itemId]);
            }
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('mutransportmodule_table_display', $urlArgs);
                }
