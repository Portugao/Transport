 ...
 *
 * This is the base repository class for table entities.
 */
abstract class AbstractTableRepository extends EntityRepository
{
     ...
     */
    protected $mainEntityClass = 'MU\TransportModule\Entity\TableEntity';
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'MUTransportModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'tables', 'userid' => $userId];
     ...
     *
     * @return array|tableEntity Retrieved data array or tableEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved tableEntity instances
     */
    public function selectByIdList($idList = [0], $useJoins = true, $slimMode = false)
    {
        ...
        if (!$slimMode && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('table', $qb);
        }
    ...
    public function getListQueryBuilder($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if (!$slimMode && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addCommonViewFilters('table', $qb);
        }
    ...
    public function selectSearch($fragment = '', array $exclude = [], $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addSearchFilter('table', $qb, $fragment);
        }
    ...
    {
        $selection = 'COUNT(tbl.id) AS numTables';
    ...
    public function selectCount($where = '', $useJoins = false, array $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('table', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of tables to exclude (optional)
     *
     * @return boolean Result of this check, true if the given table does not already exist
     */
    ...
    {
        $selection = ', tblDatabase, tblFields';
    ...
    {
        $qb->leftJoin('tbl.database', 'tblDatabase');
        $qb->leftJoin('tbl.fields', 'tblFields');
