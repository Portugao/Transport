 ...
 *
 * This is the base repository class for database entities.
 */
abstract class AbstractDatabaseRepository extends EntityRepository
{
    /**
     * @var string The main entity class
     */
    protected $mainEntityClass = 'MU\TransportModule\Entity\DatabaseEntity';
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'MUTransportModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'databases', 'userid' => $userId];
     ...
     *
     * @return array|databaseEntity Retrieved data array or databaseEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved databaseEntity instances
     */
    public function selectByIdList($idList = [0], $useJoins = true, $slimMode = false)
    {
        ...
        if (!$slimMode && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('database', $qb);
        }
    ...
    public function getListQueryBuilder($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if (!$slimMode && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addCommonViewFilters('database', $qb);
        }
    ...
    public function selectSearch($fragment = '', array $exclude = [], $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addSearchFilter('database', $qb, $fragment);
        }
    ...
    {
        $selection = 'COUNT(tbl.id) AS numDatabases';
    ...
    public function selectCount($where = '', $useJoins = false, array $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('database', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of databases to exclude (optional)
     *
     * @return boolean Result of this check, true if the given database does not already exist
     */
    ...
    {
        $qb->leftJoin('tbl.tables', 'tblTables');
