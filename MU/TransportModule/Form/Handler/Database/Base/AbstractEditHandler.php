...
namespace MU\TransportModule\Form\Handler\Database\Base;

use MU\TransportModule\Form\Handler\Common\EditHandler;
use MU\TransportModule\Form\Type\DatabaseType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        $this->objectTypeCapital = 'Database';
        $this->objectTypeLower = 'database';
        ...
        if ('create' == $this->templateParameters['mode']) {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->requestStack->getCurrentRequest()->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the database yet as other items are required which must be created before!'));
    ...
    {
        return $this->formFactory->create(DatabaseType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of database area
        $codes[] = 'userIndex';
        // admin index page of database area
        $codes[] = 'adminIndex';
        ...
        $codes[] = 'adminOwnView';
        // user detail page of treated database
        $codes[] = 'userDisplay';
        // admin detail page of treated database
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        if ($objectIsPersisted) {
            // redirect to the detail page of treated database
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ('create' == $this->templateParameters['mode']) {
                    $message = $this->__('Done! Database created.');
                } else {
                    $message = $this->__('Done! Database updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Database deleted.');
                break;
            default:
                $message = $this->__('Done! Database updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUTransportModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'database', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
